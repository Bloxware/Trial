...
-- Just to check another Bloxware instance is running and close it
...
do
    ...
    settings.fileName = "Bloxware_v8_settings.txt" -- Lovely
    settings.saved = {}
...
-- Aimbot aim bot aim aim stuff bot
...
do
    ...
    ESP.enabled = settings:Get("ESP.enabled", true)
    ...
    local hashes = {}
    function ESP.step()
        --if ESP.enabled ~= true then return clearDrawn() end
        if completeStop then return end
        ...
        if aimbot.enabled and aimbot.fovenabled and ESP.enabled then
        ...
        if ESP.enabled and crosshair.enabled then
...
do
    ...
    ah8.enabled = true
    function ah8:close()
        spawn(function() pcall(ESP.End, ESP) end)
...
local Aimbot = hud:AddTab({
	Text = "Trial",
})


local AimbotToggle = Aimbot:RemoveToggleCategory({
	Text = "Aimbot",
...
-- settings stuff
local AimbotSettings = Aimbot:AddCategory({
	Text = "Settings",
})

AimbotSettings:AddLabel({
    Text = "(Bloxware) decrease sens if aimbot is glitchy"
})
...
local FieldOfView = Aimbot:RemoveToggleCategory({
    Text = "fov",
...
local Boxes = ESP:AddToggleCategory({
    Text = "Boxes",
    State = boxes.enabled,
}, function(new)
    boxes.enabled = new
...
local Esp = ESP:AddToggleCategory({
    Text = "Esp",
    State = esp.enabled,
}, function(new)
    esp.enabled = new
...
--
local Tracers = ESP:AddToggleCategory({
    Text = "Tracers",
...
local Crosshair = ESP:AddToggleCategory({
    Text = "Crosshair",
